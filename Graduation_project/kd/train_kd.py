...
def main():
	...
	logging.info('Teacher param number = %d, Student param numbers = %d',total_num = sum(p.numel() for p in tnet.parameters()),total_num = sum(p.numel() for p in snet.parameters()))
	logging.info('-----------------------------------------------')
	...
	dataloaders_dict = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=args.batch_size, shuffle=True, num_workers=4) for x in ['train', 'test']}
